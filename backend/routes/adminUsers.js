...
router.post('/', upload.single('profilePicture'), async (req, res) => {
	...
	try {
		...
		const query = `
      INSERT INTO admin_users (full_name, username, email, password, user_role, created_by_user_id, user_avatar)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
...
router.get('/u/:id', async (req, res) => {
	...
	try {
		...
		const query = 'SELECT `user_id`, `username`, `email`, `full_name`, `user_role`, `user_registered_date`, `user_avatar`, `user_bio`, `created_by_user_id` FROM admin_users WHERE user_id = ?';
...
// Define a route to get all admin_users data
router.get('/', async (req, res) => {
	try {
		// Your SQL query to select all admin_users data
		const query = 'SELECT `user_id`, `username`, `email`, `full_name`, `user_role`, `user_registered_date`, `user_avatar`, `user_bio`, `created_by_user_id` FROM admin_users';
		// Execute the query using the promise-based API of mysql2
		const [results] = await pool.execute(query);
		// Return the admin_users data as JSON
		res.status(200).json(results);
	} catch (error) {
		console.error('Error retrieving admin_users data:', error);
		res.status(500).json({ error: 'An error occurred while fetching admin_users data' });
	}
});
...
router.get('/me', async (req, res) => {
	try {
		...
		const [user] = await pool.execute('SELECT * FROM admin_users WHERE user_id = ?', [decodedToken.userId]);
...
router.put('/:id', upload.single('profilePicture'), async (req, res) => {
	...
	try {
		// Check if the admin_users with the given ID exists
		const [checkResult] = await pool.execute('SELECT user_id FROM admin_users WHERE user_id = ?', [userId]);
		if (checkResult.length === 0) {
			return res.status(404).json({ error: 'users not found' });
		}
		// Your SQL query to update the users
		const updateQuery = `
      UPDATE admin_users SET full_name = ?, username = ?, email = ?, user_role = ?, user_avatar = ? WHERE user_id = ?
    `;
